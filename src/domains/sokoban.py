...
#             self._puzzle[self._y_man][self._x_man - 1][self._channel_man] = 1
    ...
    def predecessors(self):
        ...
        if self._x_man + 1 < self._width:
            ...
            if (self._maze[self._y_man][self._x_man + 1][self._channel_walls] == 0 and
                self._x_man - 1 >= 0 and
                self._boxes[self._y_man][self._x_man + 1] == 0 and
                self._boxes[self._y_man][self._x_man - 1] == 1):

                actions.append(self._EB)
        ...
        if self._x_man - 1 > 0:
            if (self._maze[self._y_man][self._x_man - 1][self._channel_walls] == 0 and
                ...
                actions.append(self._W)
            if (self._maze[self._y_man][self._x_man - 1][self._channel_walls] == 0 and
                self._x_man + 1 < self._width and
                self._boxes[self._y_man][self._x_man - 1] == 0 and
                self._boxes[self._y_man][self._x_man + 1] == 1):

                actions.append(self._WB)
        if self._y_man + 1 < self._height:
            if (self._maze[self._y_man + 1][self._x_man][self._channel_walls] == 0 and
                    self._boxes[self._y_man + 1][self._x_man] == 0):
            ...
            if (self._maze[self._y_man + 1][self._x_man][self._channel_walls] == 0 and
                self._y_man - 1 >= 0 and
                self._boxes[self._y_man + 1][self._x_man] == 0 and
                self._boxes[self._y_man - 1][self._x_man] == 1):

                actions.append(self._SB)
        ...
        if self._y_man - 1 > 0:
            if (self._maze[self._y_man - 1][self._x_man][self._channel_walls] == 0 and
                ...
                actions.append(self._N)
            if (self._maze[self._y_man - 1][self._x_man][self._channel_walls] == 0 and
                self._y_man + 1 < self._height and
                self._boxes[self._y_man - 1][self._x_man] == 0 and
                self._boxes[self._y_man + 1][self._x_man] == 1):

                actions.append(self._NB)

        return actions

    def inverse_apply_action(self, action):

        if action == self._N or action == self._NB:
            if action == self._NB:
                self._boxes[self._y_man + 1][self._x_man] = 0
                self._boxes[self._y_man][self._x_man] = 1
            #self._puzzle[self._y_man][self._x_man][self._channel_man] = 0
            #self._puzzle[self._y_man - 1][self._x_man][self._channel_man] = 1
            self._y_man = self._y_man - 1

        if action == self._S or action == self._SB:
            if action == self._SB:
                self._boxes[self._y_man - 1][self._x_man] = 0
                self._boxes[self._y_man][self._x_man] = 1
            #self._puzzle[self._y_man][self._x_man][self._channel_man] = 0
        ...
        if action == self._E or action == self._EB:
            if action == self._EB:
                self._boxes[self._y_man][self._x_man - 1] = 0
                self._boxes[self._y_man][self._x_man] = 1
            #self._puzzle[self._y_man][self._x_man][self._channel_man] = 0
        ...
        if action == self._W or action == self._WB:
            if action == self._WB:
                self._boxes[self._y_man][self._x_man + 1] = 0
                self._boxes[self._y_man][self._x_man] = 1
#             self._puzzle[self._y_man][self._x_man][self._channel_man] = 0
#             self._puzzle[self._y_man][self._x_man - 1][self._channel_man] = 1
    ...
    @staticmethod
    def generate_state(states, n_steps_goal:int):
        """
        generate a STP instance by taking n steps from goal
        size: 4,9,16,25 ..
        """
        idx = int(np.random.random() * len(states))
        state = states['puzzle_' + str(idx)]
        state.print()
        print(state.is_solution())
        for i in range(state._height):
            for j in range(state._width):
                if state._maze[i][j][state._channel_goals] == 1:
                    state._boxes[i][j] = 1
                else:
                    state._boxes[i][j] = 0
        ...
        for i in range(state._height):
            for j in range(state._width):
                if state._maze[i][j][state._channel_goals] == 1:
                    ...
                    if i + 1 < state._height:
                        neighbours.append((i+1, j))
                    if j + 1 < state._height:
                        neighbours.append((i, j+1))

                    for n in neighbours:
                        x, y = n
                        if state._maze[x][y][state._channel_walls] != 1 and state._boxes[x][y] != 1:
                            start_positions.append(n)
        state._y_man, state._x_man = start_positions[int(np.random.random() * len(start_positions))]
        goal_state = state

        #now take random steps!
        assert(goal_state.is_solution())
        state = goal_state
        for i in range(n_steps_goal):
            state.print()
            actions = state.predecessors()
            actions.sort()
            ...
            if max(actions) > 3:
                actions = actions[idx:]
            state.inverse_apply_action(np.random.choice(actions))
        state.print()
        return state
